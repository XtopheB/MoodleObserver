...
Response_plot <- function(data, xvar, title) {
 ...
 # geom_vline(xintercept= 3.5, lwd = 1.5, linetype="dashed", color = "pink")+ # <<- doesn't work well when not answers have a response
  ggtitle(paste(title)) +
  coord_flip() +
  theme_minimal( ) +
...
```{r numericscale}
# We transform the factors into a numeric scale
...
data.text <- data.feedback %>%
  select(Results, Comments) %>%
   mutate_all(~gsub("use |machine |learning| learned|can
                    |course| will |data", "", .))
...
## Descriptive analmysis: Most popular words
...
## Descriptive analmysis: Word Cloud
