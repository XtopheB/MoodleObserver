...
AllCoursesRef <- c("Gender_Self_1",
                   ...
                   "SEEA_CF_2022", 
                   "SBR22", 
                   "MLOS22", "ClimateChange2023", "user_2023")
...
AllCoursesStartDate <-c("2022-05-25", "2022-01-24", "2022-06-06", "2022-07-04", "2022-08-01", "2022-09-26", "2022-11-21", "2023-01-16", "2023-03-27")
AllCoursesNbModules <-c(3,6,6,5,5,7,8, 6)
```
...
Free-text comments are probably the most valuable elements for lecturers as suggestions and relevant critics can be spotted in these comments. We have **for** different free-text questions in the feedback form for this course: 
...
## Word cloud on *Relevance* and *Efectiveness*


```{r TextRelevance}
# Create corpus
All.Comments <- rbind(data.text$share, data.text$Effectiveness_open)
...
```{r TopWordsRelevance, fig.height=6}
...
barplot(Words.R,
        main = "Most popular words", 
        sub = "Relevance* and Efectiveness",
        las = 2,
        horiz=TRUE,
        col = SIAP.color)
...
```{r TextComments}
# Create corpus
All.Comments <- rbind(data.text$Comments, data.text$FinalComments)
corpus.R <- iconv(All.Comments)
corpus.R <- Corpus(VectorSource(corpus.R))
...
```{r TopWordsComments, fig.height=6}
...
data.num %>%
  ...
  filter(xfun::is_ascii(Results)== T) %>%   # remove Chinese characters
kable( ,
