...
> After cleaning, we recorded **`r nrow(course.log)`** events of different nature for the course *`r CourseNameLong `*
...
There are currently (as of *`r format(FileDate, "%A %d %B ")`*)  **`r NbCertificates`** certified learners in this course,  out of  **`r paste0(NbLearners)`** registered participants and of  **`r nrow(StudentsAboveThreshold)`** who are above the `r GradeThreshold`  threshold (and should get the certificate).
...
> The *raw* success rate  is **`r round(100*NbCertificates/NbLearners, 1)` %**. However,  if we consider that we have *no-shows*,  the success rates among **active learners** (`r NbLearnersActive`), is  **`r round(100*NbCertificates/NbLearnersActive, 1)` %**.
...
course.log %>%
 ...
 mutate(ActID.lab = paste("Mod-", ModuleID_2,"-",str_trunc(Actname, 15, "right") )) %>%
  ...
  geom_tile()+
  scale_fill_gradientn(colours= diverge_hcl(4), trans = "log")+
...
toto <- course.log %>% 
  filter(str_detect(User.full.name, "Than")) %>%
