...
Opening3 <- as_date("2022-02-30")
Opening4 <- as_date("2022-03-06")
Opening5 <- as_date("2022-03-13")
...
> Seminar days seem to be the most active ones
...
```
 ...
 > *The graphic exhibits few access differences among activities, except webinar's videos (replays) and optional activities such as the project.  Most activities were accessed by `r MedianLearnersActivity` or more learners.*
 
```{r TableLearnersByActivity}
...
In the graphic below we highlight learners (around `r round(Grades%>% filter(GradeFinal >GradeThreshold & NbAct > 0.6*max(Grades$NbAct, na.rm = TRUE) ) %>% nrow(), -1)`) that have succeed in their tests and completed  more than 2/3 of the activities proposed on the LMS. All learners should be in that zone.  
...
p <-Grades %>%
 ...
 labs(x="Nb of activities viewed", y=" Total Grade ", 
  ...
  annotate("rect", 
           ...
           xmax = max(Grades$NbAct, na.rm = TRUE)+1,
           ymin = GradeThreshold,
           ymax = max(Grades$GradeFinal, na.rm = TRUE)+1, 
...
mutate(TotalTime = TotalTime/60) %>%
 ...
 annotate("text", x = 15, y = 3*6*60 + 200,
          color= SIAP.color,
