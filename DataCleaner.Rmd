...
MyDataFrames <- list('Global' = DTCertified%>% arrange(desc(TotalReal)),
                     'ByCountry' = CertifiedCountry%>% arrange(desc(.[[2]])),
                     'BySex' = CertifiedSex)
...
mutate(TotalTime = TotalTime/60, 
       Sucess = as.factor(ifelse(GradeFinal >= GradeThreshold, "Succeed", "Failed"))) %>%
 ggplot() +
