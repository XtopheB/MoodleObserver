...
Many different information can be extracted  from the log file: all the events, for all learners,  as well as the precise *time* of each event are recorderd. We can therefore gather information at the learner or activity level. After cleaning, we recorded **`r nrow(course.log)`** events of different nature for this course
...
> We can reasonably say that `r NbLearnersActive`  participants have been really active on this course ( *`r round(100* NbLearnersActive/NbLearners, 1)`* %)
...
There are currently (`r format(FileDate, "%a %d %B ")`)  **`r NbCertificates`** certified learners in this course,  out of  **`r paste0(NbLearners)`** registered participants and of  **`r nrow(StudentsAboveThreshold)`** who are above the `r GradeThreshold`  threshold (and should get the certificate).

**`r nrow(StudentsInWorkshop)`** participants submitted a project in the mandatory data-based project. 

The *raw* success rate  is **`r round(100*NbCertificates/NbLearners, 1)` %**, if we consider only active learners, the success rates among *active learners* is  **`r round(100*NbCertificates/NbLearnersActive, 1)` %**,
