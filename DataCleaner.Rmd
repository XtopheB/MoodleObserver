...
#### Logs File
# --> use Comma separated values when donwloading

list.logs<- list.files(path="Data/",
                       pattern=paste0("logs_",CourseName),
...
list.act<- list.files(path="Data/",
                      pattern=paste0("progress.",tolower(CourseName)),
                      full.names = TRUE,recursive = TRUE)
...
list.grades<- list.files(path="Data/",
                         pattern=paste0(CourseName," Grades"),
                         full.names = TRUE, recursive = TRUE)
...
ModStartIdx <- c(1,13,22,33,46,61,74)  # For Data Visualization
...
# For MLOS, list of activities by module (OLD version)
# course_log <- course_log %>%
...
Below is a table summarizing the repartition of the specific pedagogical activities by Module. 
...
#######  PANSY'S TABLE  USED XXXX

# Certification
# Learners <- data.Pansy %>%
#   distinct(Email.address, .keep_all=TRUE) %>%
#   # filter(Certified ==TRUE)%>%
#   select(Email.address,  Sex, Country,  Certified)%>%
#   unique()
...
There are **`r NbCertificates`** certified learners in this course out of  **`r paste0(NbLearners)`** participants. The  success rate  is **`r round(100*NbCertificates/NbLearners, 1)` %**.

The repartition by country  and by gender is the following.
...
Nb0 = Grades%>% filter(GradeFinal ==0) %>% nrow()
...
course_log %>%
  mutate(ParticularDays = ifelse(DayOfWeek ==7, "Sunday", 
                                 ifelse(DayOfWeek ==5 & DayID <30 | Date =="2021-12-16" , "Webinar day", "Regular"))  ) %>%
