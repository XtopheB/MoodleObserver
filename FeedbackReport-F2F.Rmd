...
```{r Renaming}
# robust renaming approach

data.feedback <- data.feedback %>%
  rename_with(~ "Relevance1",
              .cols = matches("*relevant.*", ignore.case = TRUE))%>%
  rename_with(~ "Relevance2",
              .cols = matches("*professional.*background", ignore.case = TRUE))%>%
  rename_with(~ "Relevance3",
              .cols = matches("scope.*training", ignore.case = TRUE)) %>%
  rename_with(~ "Relevance_comments",
              .cols = matches("comments.*relevance.*training", ignore.case = TRUE))%>%
  rename_with(~ "Effectiveness1",
              .cols = matches(".effectively.enhanced.*knowledge", ignore.case = TRUE))%>%
  rename_with(~ "Effectiveness2",
              .cols = matches(".content.*.clear.*logical", ignore.case = TRUE))%>%
  rename_with(~ "Effectiveness3",
              .cols = matches(".method", ignore.case = TRUE))%>%
  rename_with(~ "Effectiveness4",
              .cols = matches(".confidence", ignore.case = TRUE)) %>%
  rename_with(~ "Effectiveness_comments",
              .cols = matches(".comments.*.effectiveness.*", ignore.case = TRUE)) %>%
  # rename_with(~ "Improve_Comments",
  #             .cols = matches("*share*.improve*", ignore.case = TRUE)) %>%
  # rename_with(~ "Logistics_open",
  #             .cols = matches(".feedback.*.travel|logistic", ignore.case = TRUE)) %>%
  rename_with(~ "Topics_Most",
              .cols = matches(".topics.were.most*", ignore.case = TRUE)) %>%
  rename_with(~ "Topics_Not",
              .cols = matches(".topics.were.not*", ignore.case = TRUE)) %>%
  rename_with(~ "Knowledge",
              .cols = matches("knowledge*", ignore.case = TRUE)) %>%
  rename_with(~ "Other_comments",
             .cols = matches(".additional|.other.*comment*", ignore.case = TRUE))%>%
  rename_with(~ "Other_feedback",
             .cols = matches("other.feedback*", ignore.case = TRUE))
...
data.text <- data.feedback.cleanned %>%
  select(matches("open|comment|feedback", ignore.case = TRUE))


data.topic <- data.feedback.cleanned %>%
  select(matches("topic", ignore.case = TRUE))
...
We present here the  *comments* for `r  nrow(data.feedback)` responses in 3 Tables.  

## Comments on Topics (*most liked* and *not covered*)
...
name_column_open <- names(data.text)

data.text %>%
  select( name_column_open[1:2]) %>%
  kable(
    caption = paste(" Comments on Relevance and efficiency from", NbTotal, "learners"),
    booktabs = TRUE,
    longtable = TRUE,
    escape = FALSE
  ) %>%
...
# First table 
data.text %>%
  select( name_column_open[3:4]) %>%
  kable(
    caption = paste(" Other comments from", NbTotal, "learners"),
    booktabs = TRUE,
    longtable = TRUE,
    escape = FALSE
  ) %>%
...
file.copy(from = here("FeedbackReport-F2F.pdf"),
