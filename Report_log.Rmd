---
title: "Analyzing the *Machine Learning* Course  <br> *(Using the Activity Log from Moodle)* "
...
Some elements are specific to the course, such has the organisation of each Module and the number of activities proposed. After cleaning and refining the data, we have identified  **`r length(unique(course_log$Actname))`**  different *activities* (information or resources such as videos, tutorials,  forums, articles, tests, polls,..) proposed for the learners on the LMS. 

Below is table summarizing the reparation of the specific pedagogical activities by Module. 
...
We have **`r NbWomen`** women participating the course (`r round(100*NbWomen/NbLearners, 2)` % ) and a diversity of origin with people from **`r NbCountries`** different countries. We present some statistics on the origin of the learners by country. 
...
There are **`r NbCertificates`** certified learners in this course out of  **`r paste0(NbLearners)`** participants (`r round(100*NbCertificates/NbLearners, 2)` %).

The repartition by country is the following.
...
Learners %>%
 filter(Country != "" | Country != "NA") %>%
  ...
  labs(x="Country", y="Percentage of sucess in each group", 
      subtitle = paste("(date ", FileDate,")"), 
      caption = " Only countries with more than 5 participants are represented") +
...
Among the  **`r paste0(NbLearners)`** participants who visited the course, only  **`r max(foo$Q)`**  ( *`r round(100* max(foo$Q)/NbLearners, 1)`* %)   viewed or attempted a quiz at least once.  We can also note that **`r max(goo$Q)`**  ( *`r round(100* max(goo$Q)/NbLearners, 1)`* %)  participants posted at least one post in any of the **`r max(hoo$Q)`** forums proposed.   
...
myPaletteModules <- c('#b3e2cd','#fdcdac','#cbd5e8','#f4cae4','#e6f5c9','#fff2ae','#f1e2cc')
...
course_log %>%
  ...
  scale_fill_manual(values = myPaletteModules ) +
...
course_log %>%
  ...
  summarise(Learners = n_distinct(User.full.name)) %>% 
  ggplot()+
  aes(x=Date, y= reorder(ActID.lab, Learners), fill=Learners)+
  geom_tile()+
...
We noticed that *some* learners (around `r round(Grades%>% filter(GradeFinal >40 & NbAct < 21) %>% nrow(), -1)`) had very good grades in their testswhile showing a **very limited activity on the LMS** (less than a third of available).  
...
course_log %>%
  ...
  labs(x="Learners' ID", y="Number of events", fill="",
       subtitle = paste(NbLearners,"Learners (Final Date ", as.Date(as.character(FileDate),format = "%Y%m%d"),")"))+
