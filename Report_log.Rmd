---
title: "Analyzing the *Machine Learning* Course Using Moodle Data"
...
#### SPECIAL 
## Using Pansy's file for grades

data.Pansy <- readxl::read_excel("Data/MLOS_Participants list.xlsx", 
                                   sheet = "participants list")

# Adjusting 

data.Pansy <- data.Pansy %>%
  rename(
  Email.address  = "Email address") %>%
  filter(!is.na(Email.address), 
         Email.address != "" )%>%
  unique()
# * `r name_last_logs` (*Logs on the LMS^[Each row of the *LMS log file* is an event and so all the events (course viewed, post, attempt to a test, etc.) are recorded, with a time stamp, in a single log file.  One field gathers the detail of each event as a text message. This field can be parsed and information on the user, the activity and the event can be extracted.] *)
#   
```
...
Learners %>%
 filter(Country != "" | Country != "NA") %>%
  ...
  labs(x="Country", y="Percentage of sucess in each group", 
      subtitle = paste("(date ", FileDate,")"), 
      caption = " Only countries with more than 2 participants are represented") +
