...
```{r ExtractIDs,eval=FALSE}
## NOT SUR THIS IS STILL VALID/USEFULL

# Extract course ID
CourseID <- str_match(data.logs$Description, "(?i)\\bcourse with id '?\\s*(\\d+)")[,2]

###  We use User.full.name instead  of Learner ID

# # Extract Learners ID
# LearnerID <- str_match(data.logs$Description, "(?i)\\buser with id '?\\s*(\\d+)")[,2]

# Extract module ID
ModuleID <- str_match(data.logs$Description, "(?i)\\bsection number '?\\s*(\\d+)")[,2]
...
#Extract activity ID
ActID <- str_match(data.logs$Description, "(?i)\\bcourse module id '?\\s*(\\d+)")[,2]

#unique(ActID)

# Extract activity type. 
Act_type <- str_match(data.logs$Event.context,  "(?<=\\[).+?(?=\\])")[,1]

# unique(Act_type)# Many students also accessed old version such as "OLD-Video".
#CourseID is not added since it means this course
course_log <- data.frame(data.logs, ModuleID, LearnerID, ActID, Act_type)
...
> The course started on **`r CourseStartDate`**. So far (`r FileDate`), **`r paste0(NbLearner)`** different learners **visited**  the course. Many different events can be extracted  from the data as well as the precise *time* of each event. We can therefore gather information at the learner or activity level. 
...
```{r TableAct, echo=FALSE, results=TRUE}
  kable(actname, row.names = FALSE)
```
## TODO: REMOVE inappropriate Activities
...
> There are **`r NbCertificates`** certified learners in this course

## Specificity of the course (weeks)

``````{r ModuleFilter}
#### TBR!!!!


### Define the Modules for that activities 
course_log <- course_log %>%
            mutate(ModuleID_2 = case_when(ActOrder == 1 ~ -1,
                                          between(ActOrder, 4, 11) ~ 0,
                                          between(ActOrder, 12, 18) ~ 1,
                                          between(ActOrder, 19, 27) ~ 2,
                                          between(ActOrder, 28, 35) ~ 3,
                                          between(ActOrder, 36, 50) ~ 4,
                                          between(ActOrder, 51, 59) ~ 5,
                                          between(ActOrder, 60, 68) ~ 6,
                                            )) %>%
...
course_log %>%
  filter(!is.na(Event.context),
         Date <= FileDate) %>%
  mutate( ActID.lab = paste(ActOrder,"-",Event.context)) %>%
  group_by(ActID.lab) %>%
  summarise(Learners = n_distinct(User.full.name),
...
We note that **`r Grades%>% filter(GradeFinal >=42) %>% nrow()`** have a grade greater than 70% of the maximum. 
...
learners_Time <-  course_log %>%
  select(User.full.name,  Actname, ActOrder,Date, Time) %>%
...
names(Grades)[10:16] <- c("Test1", "Test2", "Test3", "Test4", "Project", "Test5", "Test6")
...
course_log %>%
  ...
  geom_bar(stat='identity')+
  labs(x="Activities", y="Number of Learners", 
       subtitle = paste(nrow(subset(unique(course_log[c("ActID", "Date")]), Date==FileDate)),"activities (Date ", FileDate,")"))+
  ggtitle("Activities accessed by number of Learners")+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        plot.title = element_text(hjust = 0.5, size=12),
        panel.background = element_blank(), text = element_text(size=8), 
