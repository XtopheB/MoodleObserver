---
title: "Analyzing the Data Visualization e-Learning Course \n *Using the Activity Log from UNSIAP LMS* "
...
This report has been **automatically generated** using the data collected on the MLS on `r as.Date(as.character(FileDate),format = "%Y%m%d")` . The first step is the data cleaning of the raw log file, or *event data set*:(*`r paste0("Data/logs_DTV20_", FileDate,".csv")`*)
...
The Moodle platform allows the recording of many events during a course. Several data sets can then be downloaded (.csv format) for an analysis of the activity of the learners. In fact all the events are recorded, with a time stamp, in a single log file. One field gathers the detail of each event. This field can be parsed and information on the user, the activity and the event can be extracted.
...
>   There are **`r length(unique(course_log$Actname))`**  different *activities* (resources such as videos, forums, articles, tests, polls, ..) in the course and reported in this log. 
...
Since there is an important time difference between the teacher and the learners, it was important to have an idea of the learner's activity on the platform by hour.  The graphic below shows the maximum number of learners for each time period over the day. 
...
```{r heatMap, fig.width=12, fig.height=25, out.width = "80%"}
# course_log %>%
...
[^MyNote]: We removed for the list the lecturers as well as some learners registered with different emails, we count here **unique** learners.  
...
[^Time]: We do not always observe when someone leaves the platform, and the duration reported here may reflect connections that lasted while the learner was doing something else.    
...
We note that **`r learners%>% filter(GradeFinal >=70) %>% nrow()`**, over the **`r learners%>% filter(Project>0) %>% nrow() +6`**   who submitted a project (mandatory condition), succeeded with a grade above 70 (over 100). This is a rate of success of **`r round(100* (learners%>% filter(GradeFinal >=70) %>% nrow()) / (learners%>% filter(Project>0) %>% nrow() +6),1)`** %.[^grades]

[^grades]: The distribution of grades excludes here the  *`r learners%>% filter(GradeFinal ==0) %>% nrow()`* participants that did not participate in the tests at all.  
