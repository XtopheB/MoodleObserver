...
### Learners prefered activities {-}

With no suprises, the top activities in terms of learners participation rank according to the modules order, with activities in module 1 on top and activities of modules 6 at the bottom. Some noticeable exceptions are the 6 quizzes, the data-based project and a homework that aremore popular than other activities of their respective modules.    
...
This *heatmap* is an attempt to visualize the activities on the platform over time.  The coduing is the following:
...
* Heat = color:  Nb of views (nb of Learners or events) for each activity 

```{r heatMap, fig.width=12, fig.height=25}
# course_log %>%
...
Over the `r nrow(learners)` unique learners[^MyNote] registered for the course, only **`r learners%>% filter(TotalTest >0) %>% nrow()`** attempted  to at least one test and only **`r learners%>% filter(NbDays > 6 ) %>% nrow()`** visited the LMS more than 6 times.  

[^MyNote]: Some learners registered with different names, we count here **unique** learners.  


We noticed that *some* learners (around `r learners%>% filter(TotalTest >50 & NbAct < 21) %>% nrow()`) had very good grades in their tests ( >50) while showing a very limited activity on the LMS (less than a third of available).  
...
mutate(TotalTime = TotalTime/60) %>%
 ...
 labs(x="Nb of activities viewed", y=" Total Time spent by learners (min)", 
      subtitle = paste(nrow(subset(unique(course_log[c("ActID", "Date")]),
                                   Date==final_date)),"activities (date ", final_date,")"))+
...
We note that **`r learners%>% filter(GradeFinal >=70) %>% nrow()`** , over the **`r learners%>% filter(Project>0) %>% nrow() +6`**   who submitted a project, succeeded with a grade above 70 (over 100). 

> This is a rate of ** `r round(100* (learners%>% filter(GradeFinal >=70) %>% nrow()) / (learners%>% filter(Project>0) %>% nrow() +6),1)` %.

The distribution of grades is the following. 
...
learners %>%
 ...
 annotate("rect", xmin = 70, xmax = 100,
                  ymin = 0, ymax = 32,
