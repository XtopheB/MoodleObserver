...
`r as.Date(paste(FileDate), format = "%Y%m%d")` 
...
```{r heatMap, fig.width=12, fig.height=19}
# course_log %>%
...
Grades <- Grades.raw %>%
  ...
  mutate(Project = ifelse(Project == "-", NA, as.numeric(as.character(Project)))) %>%
  rename_with(~NewNames[which(OldNames == .x)], .cols = OldNames) %>%
  mutate_at(vars(starts_with("Test")),funs(as.character)) %>%
  mutate_at(vars(starts_with("Test")),funs(as.numeric)) %>%
  select(First.name:Country, Email.address:Test6,Project) %>%
  mutate(
    TotalTest = rowSums(dplyr::across(Test1:Test6), na.rm = T), 
    GradeFinal = TotalTest + as.numeric(Project))
...
learners %>%
 ...
 ggplot() +
 aes(x = GradeFinal) +
 geom_histogram(bins = 50L, fill = SIAP.color, color = "white") +
