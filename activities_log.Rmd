...
> So far, **`r paste0(NbLearner)`** different Learners visited the course.  There are **`r length(unique(course_log$Actname))`**  different activities in the course and reported in this log. 
...
course_log %>%
  ...
  labs(x="Events", y="Number of Learners", 
       subtitle = paste(length(unique(course_log$Event.name))-1,"events (Final date ", as.Date(as.character(FileDate),format = "%Y%m%d"),")"))+
...
course_log %>%
  ...
  aes(x = as.factor(DayID),  fill = IsSunday)+
