...
# Gathering the data
There are 3 (now 4 with user information) files to gather + a list of countries and country codes that can be useful
...
> The data can be downloaded  directly from here [https://learning.officialstatistics.org/grade/export/txt/index.php?id=113](https://learning.officialstatistics.org/grade/export/txt/index.php?id=113)
...
Since the information on learner's country is not recorded by the UNSD  Moodle platform (and we cannot change that feature), we have decided to add a special field where users would enter some information on their position  and country. 

> The data has be downloaded from here [https://learning.officialstatistics.org/mod/feedback/view.php?id=1833](https://learning.officialstatistics.org/mod/feedback/view.php?id=1833)


```{r LoadUserData}
### 3-  Grade File

list.user.file<-list.files(path="DataCEBD/",pattern = glob2rx("*User*"),
                        full.names = TRUE,recursive = TRUE)

name_last_user_file <- list.user.file[which.max(file.mtime(list.user.file))]

# Download the latest Grades files
data.user <- read.csv(name_last_user_file)

```

**Note** that not all the participants have recorded this information. Currently, we have this information for `nrow(name_last_user_file)`participants 
...
-  `r name_last_user_file ` (*latest user information* <- NEW)
...
Administrators and teachers of this course have to be  removed from the analysis.
...
data.logs <- data.logs[!( data.logs$User.full.name=="Christophe Bontemps" |
                          ...
                          data.logs$User.full.name=="Ceri Regan" |
                          data.logs$User.full.name=="Rachael Beaven" |
...
```{r MergeUserInformation}

# Cleaning Information that is redundant for merging
data.user <-  data.user %>%
  select(-c(User.full.name, Date))

# Merging logs with Information from users   
  
data.logs <- left_join(data.logs ,data.user, by = c("Email.address"))
...
The course was opened for registration on December 2023, since then the number of participants has been regularly increasing, with some peaks for specific actions in some countries. 
...
No information on Gender is available from the user data set, but using the user information file, we can identify learner's origins. 
...
> Participants from **`r NbCountries`** countries have already joined the course
...
#removing when large differences (a country having lots of learners) 
...
Learners <- data.logs %>%
  ...
  ungroup() %>%
  distinct(User.full.name, .keep_all=TRUE) %>%
  select(User.full.name, Email.address, Country,  Institution, Department, TotalReal, Certified, IP.address, Time) %>%
   filter(User.full.name != "-")

Learners$Country <- ifelse(is.na(Learners$Country), "Unknown", Learners$Country)
...
##  Certified by country

```{r CertifiedCountries}

certified_country <-  Learners %>%
  filter( Certified==TRUE) %>%
 group_by(Country) %>%
  summarise(Learners = n_distinct(Email.address)) %>%
  arrange(desc(Learners)) 
...
Only `r nrow(certified_country)-1` countries have certified participants. This number is a minimum since some participants from other countries (*unknown* here) have completed the course.   

```{r TableCountries}

# Better looking if long list of Countries
kable(certified_country, "latex", longtable = T, booktabs = T) %>%
   kable_styling(latex_options = c("repeat_header"), font_size = 7)

```

The success rate (*i.e.* the Nb of certified/ Nb of participants) can be analysed by country.
