...
```
 ...
 > *The graphic exhibits a few access differences among the **`r length(unique(course.log$Actname))`** 
...
In the graphic below,  we emphasized **in a box** participants that have succeed in their tests and completed  more than 2/3 of the activities proposed on the LMS. All learners should be in that zone, and we found in that zone   **`r round(Grades%>% filter(GradeFinal >GradeThreshold & NbAct > 0.6*max(Grades$NbAct, na.rm = TRUE) ) %>% nrow(), 1)`** (over `r NbCertificates` certified).   
