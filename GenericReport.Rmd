...
StringIdentiyingGrades <- "grand"  # Can be "total"
...
actname <- actname %>%
          ...
          mutate(ModuleID = ifelse(grepl("feedback|certificate", Activity, ignore.case = TRUE), NA, ModuleID))%>%
    filter(!is.na(ModuleID))
...
With that definition, excluding the "*no-shows*", the success rate in the Asia-Pacific is for women  `r  SuccessRatesP$Rate[SuccessRates$Sex == "Female"]` \% , while for men it is  `r SuccessRatesP$Rate[SuccessRates$Sex == "Male"]` \%.
