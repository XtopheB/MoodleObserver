...
## Manual Modules definition  $
# -->   use DF: actname created later --> Chunk activity 
ModStartIdx <- c(2, 13,  22, 35, 45)  # Index of the start for each module


##  Define the string that identify the "Final" grade used for certification 
# --> Use DF: Grades  for definition 
...
course.log <- course.log %>%
  group_by(Date) %>%
  mutate(DayID = cur_group_id()) %>%
  ungroup() %>%
  mutate(
     TimeID    = as.numeric(str_extract(Time_range, "([\\d.]+)")),
...
# removing  information, etc...
actname <- actname %>%
  filter(ModuleID <= NbModules)


## For Admin data course we need +1
# actname <- actname %>%
#             mutate(ModuleID = ModuleID +1)
...
# Counting participants from Myanmar
...
LearnersByActivity <- course.log %>%
  ...
  filter(ModuleID != "NA") %>%       # there may be still some issues with the module
...
myPalette <-  myPaletteModules6

course.log %>%
  filter(!is.na(Event.context)) %>%
  filter(Actname != CourseNameLong) %>%
  #filter(ModuleID != "NA") %>%       # there may be still some issues with the module
  filter(ModuleID >= 0 & ModuleID <= NbModules & !is.na(ModuleID) )%>% 
  filter(str_detect(Event.name, "viewed")) %>%  # Removing Completion without views!!
  mutate(
...
course.log %>%
 ...
 # aes(x=Date, y=fct_rev(fct_infreq(ActID.lab)), fill=Learners)+
  ...
  labs(x="Date", y="Activities", 
       ...
       caption = " Dark red vertical lines indicate day of intense activity over all avaialble activities")+
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.title.y = element_text(size = 8))+
  coord_fixed(ratio = 0.6)  # Width/height ratio: can be  0.2 -  1.5
```
...
p <-Grades %>%
 ...
 labs(x="Nb of activities viewed", y=" Total Grade ", 
      ...
      caption=  paste("Successful learners are in the box\n","Date", FileDate) )+
...
learners_Time %>%
  ...
  geom_vline(xintercept = MinutesQ50, linetype = "dashed", color = "red", size = 1) +
   annotate("text", x = 2*MinutesQ50 , y = NbCertificates/10,
          color= "red",
...
With **`r NbCertificates` certified learners** from **`r NbCountries` different countries**, out of  `r paste0(NbLearners)` registered participants and  **`r paste0(NbLearners -NbStudentsNoShow)`** active participants -- excluding  `r NbStudentsNoShow` participants with no activity the course --   the course has certainly been useful for many.

**`r BestCountry`** with  **`r NbBestCountry`** certified participants represented the vast majority of our learners (`r  round((100* as.numeric(NbBestCountry)/NbCertificates), 2)` %).  
...
```
 <!--  TIP: Ctrl+shif+C for comments -->

<!-- The **mandatory** data-based exercise has been completed by  **`r NbProjects`** participants and peer-reviewed. -->

The course was quite long (`r as.numeric(CourseDuration)` days), and we have  experienced some attrition, but a good engagement in forums. 
<!-- Many participants shared their resources (ML case-studies), providing us with some new materials to examine. Finally, the  success rate is  **`r RealSuccessRate`** %  and similar to previous editions. -->
