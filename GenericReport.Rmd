...
# -- Data technical details  (*for internal use*)--
...
CourseDuration <- AllCourses %>%
  filter(Id == i.course) %>%
  mutate(DurationDays = dmy(Ends) - dmy(Starts),
        DurationWeeks = interval(dmy(Starts), dmy(Ends)) / dweeks(1) )%>%
  pull(DurationDays)
...
## Manual Modules definition  -->   use actname created later
ModStartIdx <- c(1,10,27,36, 45, 57,67)  # Parameters


##  Define the string that identify the "Final" grade used for certification 
StringIdentiyingGrades <- "Total"  # Can be "total"
...
# GradeMax = 100  # 4 tests with max 10
# PercentThreshold = 70   # 70% is the norm for tests
# GradeThreshold <-GradeMax * (PercentThreshold/100)  
# 
# # Dates parameters
#  WebinarDay = 2 # Sunday is 1, wednesday is  4
...
# Dates of Modules opening User_2024
# Opening1 <- as_date("2023-08-12")
# Opening2 <- as_date("2023-08-12")
# Opening3 <- as_date("2023-08-19")
# Opening4 <- as_date("2023-08-19")
# Opening5 <- as_date("2023-08-26")
 
# Dates of Modules opening DTV24 
# Opening1 <- as_date("2024-11-11")
# Opening2 <- as_date("2024-11-11")
# Opening3 <- as_date("2024-11-18")
# Opening4 <- as_date("2024-11-25")
# Opening5 <- as_date("2024-12-02")
# Opening6 <- as_date("2024-12-02")
...
# ModStartIdx <- c(1,8,17,28,38)  # Parameters for RAP2024 - 5 modules !
# ModStartIdx <- c(1,13, 22, 33, 46, 61, 74)  # Parameters for DTV24 - 6 modules !
#  Nothing for for User2024 - 7 modules !
# ModStartIdx <- c(1,2,3,4,5,6,7,8)  # Parameters for Admin Data - 6 modules !
# ModStartIdx <- c(1,9,18, 28, 37, 48)  # Parameters for RAP 2025!
...
country_df <- read.csv("SourceData/UNSD-CountryCodes.csv",
                       sep = ";" ,
                       fileEncoding = "UTF-8")

# Select relevant columns 
country_df <- country_df %>% 
select(ISO.alpha2.Code, Country.or.Area, Region.Name)%>%
...
list.logs<- list.files(path="Data/",pattern = glob2rx(paste0("logs*",CourseName,"*.csv")),
...
-  `r name_last_logs` ($\rightarrow$ *Logs on the L MS[^FileNote]*)
-  `r name_last_act `  ($\rightarrow$ *Learners activities*)
-  `r name_last_grades ` ($\rightarrow$ *Grades*)


[^FileNote]: Each row of the *LMS log file* is an event and so all the events (course viewed, post, attempt to a test, etc.) are recorded, with a time stamp, in a single log file.  One field gathers the detail of each event as a text message. This field can be parsed and information on participants, grades, certificates and activities can be extracted.
...
data.logs <- data.logs[!( data.logs$User.full.name=="Christophe Bontemps" |
                          ...
                          data.logs$User.full.name=="Jane Doe" |  # <- Sinovia's fake student
                          data.logs$User.full.name=="Sokol Vako" |
                          data.logs$User.full.name=="Chesca Rosales" |
                          data.logs$User.full.name=="Pinar Ucar" |
                          data.logs$User.full.name=="Shuji Fukuyama"|
                          data.logs$User.full.name=="ian banda" |
                          data.logs$User.full.name=="Jeroen Minderman"|
                          data.logs$User.full.name=="Hugo Pigott"|  
                          data.logs$Component=="Recycle bin"),]

# Remove everything linked to me 
# (as admin my name may appear for a specific user task like certificates! )
library(stringr)
data.logs <- data.logs %>%
   filter(!if_any(everything(), ~ str_detect(as.character(.), "Bontemps")))
...
# ***`r CourseNameLong `***: Main figures

The course *`r CourseNameLong `* started on **`r format(as.Date(CourseStartDate, format = "%d/%m/%Y"),  "%A %d %B")`.**  
...
> As of `r format(FileDate, "%A %d %B")`, **`r paste0(NbLearners)`** different learners **visited**  the course.^[This number can be smaller than the number of people who registered since it only counts **visitors**. People who did not ***visit*** the LMS are thus not counted. We use the term *participants* to identify these *visitors* all over this document.]  
...
## Removing anything after ":" in activity name
## Compliance  with course_ log later on 
actname <-  actname %>%
  mutate(Activity = str_split(actname$Activity, ": ",
                             simplify = TRUE)[,1] )
...
actname <- actname %>%
          ...
          mutate(ModuleID = ifelse(grepl("feedback|certificate", Activity, ignore.case = TRUE), NA,
                                   ModuleID))%>%
    filter(!is.na(ModuleID))

# For user engagement (since the activity list is not complete)
# actname <- course.log  %>%
#   arrange(Time) %>%
#   select(Event.context) %>%
#   mutate(Activity = str_split(course.log$Event.context, ": ",
#                              simplify = TRUE)[,2]) %>%
#   distinct(Activity) %>%
#   mutate(ModuleID = as.numeric(str_extract(Activity, "\\d+")), 
#          ModuleID = ifelse(is.na(ModuleID), 0, ModuleID), 
#          ModuleID = ifelse(str_detect(Activity, "Peer"), 5, ModuleID) , 
#          ActNb = ModuleID) %>%
#   filter(ModuleID < NbModules & ModuleID >0) 
...
```{r ModuleNb}
# Matching and adding columns- Including activity name and ModuleId
## ACTUNG: Does not work for activities with : in the text!!!
...
Each course is specific in its form and in the organisation of each Module and the number of activities proposed. After cleaning and refining the data, we  identified  **`r NbActivities`**  different *activities* proposed for the learners in *`r NbModules`* modules.^[An additional module, *M0*, is sometimes proposed with documents, prerequisites and Ice-Breaker or a forum.] 

Below is a table with the **`r NbActivities`** different pedagogical activities proposed in the course, excluding pdfs and other resources to download. 


```{r TablePrint}
# module.tab
# Better looking if long list of Modules
ModuleToPrint <- module.tab
# %>%
#   select(ModuleID, ActNb, Activity)

kable(ModuleToPrint, "latex", longtable = T, booktabs = T) %>%
...
actname %>%
  mutate(
    Actnumber = str_extract(Activity, "^[^\\[]+"),  # Extract elements before '['
    Type = str_extract(Activity, "(?<=\\[)[^\\]]+"),  # Extract elements between '[' and ']'
    Title = str_extract(Activity, "(?<=\\])[^\n]+")  # Extract elements after ']'
  )%>%
  select(ModuleID, Actnumber, Type, Title) %>%
  # save the current structure of the course in an excel file 
  write.xlsx(.,
           file=paste("Output/",CourseName,"-Activities-",ymd(Sys.Date()),".xlsx", sep=""))
...
```{r GradeVar}
# Step 2: Adding Sex and Country  to course.log using data.grades 
# !!!! Need to adjust name of Total grade (sometimes Total..Real,  Course.total..Real. ,  Grand.Total..Real.)

# Detect which columns contain the search string
matched_col <- names(data.grades)[str_detect(names(data.grades), regex(StringIdentiyingGrades, ignore_case = TRUE))]

data.grades <- data.grades %>%
  mutate(TotalReal =  as.numeric(data.grades[,matched_col]))

# data.grades <- data.grades %>%
#   #mutate(TotalReal =  as.numeric(Course.total..Real.)) %>%
#   mutate(TotalReal =  as.numeric(Quiz..Final.Test..Real.) *10)  #" Some course the total is on 10
...
```{r Remove Myanmar, include=FALSE}
# Counting particpants from Myanmar
distinct_users_my <- course.log %>%
  filter(Country == "MY") %>%
  distinct(User.full.name) %>%
  nrow()

if (distinct_users_my > 0) {
  print(paste("Filtering out", distinct_users_my, "participants from Myanmar"))
} else {
  print("")
}

# Removing country for participants from Myanmar
course.log <- course.log %>%
  mutate( Country = ifelse(Country =="MM", "", Country))
...
temp_country <- course.log %>%
  ...
  mutate(  differ = Learners[1] - Learners)%>% 
...
temp_country <- temp_country %>%
  mutate(Region = ifelse(Region == "Oceania", "Pacific", Region))


#removing when large differences 
cond <- max(temp_country$differ, na.rm = TRUE)>100

# Extracting name of large country t
...
```{r Messages}
# Sometimes one want to remove countries with less than XXX learners
MinNbLearners <- 3

# Automatic messages
# Few learners by countries
Message1  <- paste("For better readability, data from", temp_country$Country[1], "(",temp_country$Learners[1], "obs) has been removed\n")
                   

# Countries with too many participants 
Message2 <- paste("Countries with less than",MinNbLearners+1,"participants have been removed\n")

# Full message
message <- ifelse(cond,paste(Message1, Message2), Message1 )
...
# Graphic 
temp_country %>%
 dplyr::slice(if (cond) -1 else 1:n()) %>%  # to remove "large" countries 
filter(Learners>MinNbLearners) %>%           # to remove "small" countries
 ggplot() +
 aes( x=reorder(Country, Learners), y=  Learners) +
 geom_bar(aes(fill = Country),stat='identity', alpha = 0.3)+
geom_text(aes(label= Learners, color = Country), vjust= 0.4, size=3.5,)+
 scale_fill_hue() +
 ggtitle(paste("Learners by country, (Total",NbCountries , "countries)")) +
...
## Attendance in the Asia-Pacific region. 


```{r  RegionParticpants}
PacificAttendance <- temp_country %>%
  filter(Region %in% c("Asia", "Pacific") ) %>%
  group_by(Region, Country) %>%
  dplyr::summarize(Total_Learners = sum(Learners, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(desc(Total_Learners))

NbAPLearners <- PacificAttendance %>%
  summarise(Nblearners = sum(Total_Learners))

```

In the Asia-Pacific region, we have **`r nrow(PacificAttendance)`** different countries, representing `r NbAPLearners$Nblearners`  learners (`r round(100*NbAPLearners$Nblearners/NbLearners,1)`  \%). More details in the table below. 

```{r  TableRegion}
# Generate the table with grouped rows by Region
PacificAttendance %>%
  arrange(Region, desc(Total_Learners) ) %>%
  kbl(col.names = c("Region", "Country", "Total Learners"), align = "llr", longtable = T) %>%
  kable_classic(full_width = FALSE, ) %>%
  pack_rows(index = table(PacificAttendance$Region))
```
...
# Danger zone!!!! Automatic  sometimes multiple match...
# Grades$GradeFinal <- as.numeric(Grades[ , grep(c("Total","total", "Final"), colnames(Grades))])  # Find matches
Grades$GradeFinal <- as.numeric(Grades$TotalReal)
...
# Certification
Learners <- course.log %>%
  distinct(User.full.name,Email.address, .keep_all=TRUE) %>%
...
```{r NBCertified}
NbCertificates <- nrow(subset(Learners, Certified == TRUE & !is.na(TotalReal)))
```
...
learners_Time <-  course.log %>%
  ...
  filter(!grepl("feedback|certificate|Announcements", Actname, ignore.case = TRUE)) %>%
  mutate(RealTime = lubridate::dmy_hms(Time)) %>%
...
```{r CountryAnalysis}
NbCountries <- Learners %>%  
  filter(Certified==TRUE) %>%
  select(Country)%>%
  group_by(Country) %>% 
  n_distinct()
  
CertifiedByCountry <-  Learners %>%  
  filter(Certified==TRUE) %>%
  group_by(Country) %>% 
  count() %>%
  arrange(desc(n))
...
# Extracting for text
BestCountry <- paste(CertifiedByCountry[1,1]) 
NbBestCountry <- paste(CertifiedByCountry[1,2])

```

There are currently (as of *`r format(FileDate, "%A %d %B ")`*)  **`r NbCertificates` certified learners** in this course,  out of  **`r paste0(NbLearners)`** registered participants and of  **`r paste0(NbStudentsAboveThreshold)`** who are above the `r PercentThreshold` % threshold (and are eligible for a certificate). 

Certified people come from **`r NbCountries` different countries**. The country with most certified participants is **`r BestCountry`** with  **`r NbBestCountry`** certified participants.
...
DTCertified <- data.table(Learners %>%  filter(Certified==TRUE & !is.na(TotalReal)))
...
```{r}
rm("CertifiedCountryL", "CertifiedCountryW")
```


##  Success rate

The success rate can be analysed by country and gender.
...
Learners %>%
 filter(Country != "" | Country != "NA")%>%
  ...
  labs(x="Gender", y="Percentage of sucess", 
      caption = paste(NbStudentsNoShow, " participants with a grade of 0 (`no-shows`) excluded \n
                     (date:", format(FileDate, "%A %d %B"),")"))+
  theme_minimal()+
 # geom_hline( yintercept = RealSuccessRate/100,  color = "red" ) +
  theme(legend.position = "bottom")+ 
...
```{r SuccessRates}
LearnersWithoutNoShows <-Learners %>%
  filter(Country != "" | Country != "NA")%>%
  filter(TotalReal > 0 | !is.na(TotalReal)) 

SuccessRates <- LearnersWithoutNoShows %>%
  group_by(Sex) %>%
  summarise(
    Total = n(),
    Certified_Count = sum(Certified),
    Rate = round((Certified_Count / Total) * 100, 1)
  ) %>%
  select(Sex, Rate)

#SuccessRateW <- SuccessRates$Rate[SuccessRates$Sex == "Female"]

```

With that definition, excluding the "*no-shows*", the success rate for women is `r  SuccessRates$Rate[SuccessRates$Sex == "Female"]` \% (`r SuccessRates$Rate[SuccessRates$Sex == "Male"]` \%for men)
...
Learners %>%
 ...
 filter(TotalReal > 0 | !is.na(TotalReal)) %>%
  ...
  aes(x = reorder(Country, PercentC) , y = One , fill = Certified)  +
  geom_bar(position="fill", stat="identity",  alpha = 0.5) +
  ggtitle("Sucess rate by country in the Asia-Pacific") +
  labs(x="Country", y="Percentage of sucess", 
      caption = paste("(date:", format(FileDate, "%A %d %B"),")"))+
  guides(fill = guide_legend(title = "Sucess")) +
  theme_minimal()+
   # annotate("text", x = 1, y = RealSuccessRate/100,
   #        color= "red",
...
```{r GraphSuccessBySexP, fig.height= 3}
Learners %>%
  filter(Region %in% c("Asia", "Pacific") ) %>%
 filter(Country != "" | Country != "NA")%>%
  filter(TotalReal > 0 | !is.na(TotalReal)) %>%
  ...
  aes(x = Sex , y = One , fill = Certified) +
  geom_bar(position="fill", stat="identity",  alpha = 0.5) +
  ggtitle("Sucess rate by gender in the Asia-Pacific") +
  labs(x="Gender", y="Percentage of sucess", 
      caption = paste("(date:", format(FileDate, "%A %d %B"),")"))+
  theme_minimal()+
 # geom_hline( yintercept = RealSuccessRate/100,  color = "red" ) +
...
```{r SuccessRatesP}
LearnersWithoutNoShowsP <-Learners %>%
  filter(Region %in% c("Asia", "Pacific") ) %>%
  filter(Country != "" | Country != "NA")%>%
  filter(TotalReal > 0 | !is.na(TotalReal)) 

SuccessRatesP <- LearnersWithoutNoShowsP %>%
  group_by(Sex) %>%
  summarise(
    Total = n(),
    Certified_Count = sum(Certified),
...
With that definition, excluding the "*no-shows*", the success rate in the Asia-Pacific is for women  `r  SuccessRatesP$Rate[SuccessRatesP$Sex == "Female"]` \% , while for men it is  `r SuccessRatesP$Rate[SuccessRatesP$Sex == "Male"]` \%.
...
GradesPositive %>%
 ...
 labs(x="Grades", y=" Nb of sucessful learners",
      caption = paste("(Excluding",NbStudentsNoShow, "participants with grade = 0) \n
                     (date:", format(FileDate, "%A %d %B") ,")"),
      subtitle = paste0(NbStudentsAboveThreshold," learners reached the threshold") )+
 annotate("rect", xmin = GradeThreshold, xmax = GradeMax+1,
                  ymin = 0, ymax = Ymax,
                  fill = SIAP.color , alpha = .2)+
 annotate("text", x = 1.2 * GradeThreshold, y = Ymax*0.75,
        color= SIAP.color,
        label = "Success")+
 theme_minimal()
...
GradesPositive %>%
 filter(Country != "" | Country != "NA")%>%
  ...
  labs(x="Final Grades", y=" Sex ", 
      subtitle = paste("(date:", format(FileDate, "%A %d %B "),")")) +
    theme_ridges(grid = FALSE) +
  geom_vline( xintercept = GradeThreshold,  color = "red" ) +
  annotate("text", x = 1.2 * GradeThreshold , y = 0.75,
          color= "red",
          label = "Success")+
  theme_minimal()+
...
GradesPositive %>%
 filter(Country != "" | Country != "NA")%>%
  ...
  labs(x="Final Grades",
    ...
    theme_ridges(grid = FALSE) +
  geom_vline( xintercept = GradeThreshold,  color = "red" ) +
   annotate("text", x = 1.1 * GradeThreshold, y = 0.75,
          color= "red",
          label = "Success")+
  #theme_minimal()+
  theme(legend.position = "none")
...
course.log %>%
  ...
  filter(Date >= as.Date(CourseStartDate, format = "%d/%m/%Y")) %>%
  ggplot()+
...
LearnersByActivity <- course.log %>%
  ...
  # mutate(Actname = paste(ActNb, "-", Actname)) %>%
  # filter(ActNb <= NbModules )%>%
  group_by(ActID.lab) %>%
  summarise(Learners = n_distinct(User.full.name),
...
```{r GraphBarActivities, fig.height=10, fig.width=10, out.width = "80%"}
 ...
 myPaletteModules7 <- c('#fdbf6f', '#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#e31a1c')
# myPaletteModules6 <- c( '#fdbf6f',  '#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99')
# myPaletteModulesDark <- c("grey", '#a6cee3','#1f78b4','#b2df8a','#33a02c','#fb9a99','#fdbf6f')

# myPaletteModules <- c("lightgrey",'#a6cee3',  '#b2df8a','#fb9a99', "purple")
# myPaletteModulesDark <- c("grey",'#1f78b4','#33a02c','#e31a1c', "purple")

myPalette <-  myPaletteModules7

course.log %>%
  filter(!is.na(Event.context)) %>%
  filter(Actname != CourseNameLong) %>%
  #filter(ModuleID >= 0 & ModuleID <7 & !is.na(ModuleID) )%>% 
  filter(str_detect(Event.name, "viewed")) %>%  # Removing Completion without views!!
  mutate(
    # removing right then left parts of full activities name
    ActShort = str_trunc(Event.context, 35, "right"), 
    ActShort = ifelse(str_detect(Event.context, "\\["), str_extract( ActShort, "\\[.*"), ActShort),       
    ActID.lab = paste0(ActNb,"-",ActShort) ) %>%
  group_by(ActID.lab) %>%
...
```
 ...
 > *The graphic exhibits a few access differences among the **`r nrow(actname)`** 
original activities proposed, showing a good commitment of active participants*.^[For the webinars, the numbers reports the views of the webinar's **recording**, not the number of particpant who attended live.]  
...
course.log %>%
 ...
 filter(Date >= as.Date(CourseStartDate, format = "%d/%m/%Y")) %>%
  ...
  mutate(ActID.lab = factor(ActID.lab, levels = unique(ActID.lab[order(ActNb)]))) %>% # Order 
  group_by(ActID.lab, Date) %>%
  summarise(Learners = n_distinct(User.full.name)) %>% 
  ggplot()+
 # aes(x=Date, y=fct_rev(fct_infreq(ActID.lab)), fill=Learners)+
  aes(x = Date, y = fct_rev(ActID.lab), fill = Learners) +
  geom_tile( )+
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.line = element_line(colour = "black"), 
        axis.text.y = element_text(face= "plain", size=8, color="black"), # size can be  4.5
        plot.subtitle = element_text(hjust = 0.5, size=8),
        legend.position = "bottom",
        axis.title.y = element_text(size = 8))+
  coord_fixed(ratio = 1.5)  # can be  0.5
```
...
In the graphic below,  we emphasized **in a box** participants that have succeed in their tests and completed  more than 2/3 of the activities proposed on the LMS. All learners should be in that zone, and we found that   **`r round(Grades%>% filter(GradeFinal >GradeThreshold & NbAct > 0.6*max(Grades$NbAct, na.rm = TRUE) ) %>% nrow(), 1)`** were in that zone (over `r NbCertificates` certified).   
...
p <-Grades %>%
 ...
 ggplot() +
  ylim( c(0,GradeMax))+
  annotate("rect",
           xmin= NbActivities *0.8,  # Assuming participants should do  80% of activities
           xmax = max(Grades$NbAct),    
           ymin = GradeThreshold,
           ymax = GradeMax,
           fill = "grey",  alpha = 0.1,
 ...
 labs(x="Nb of activities viewed", y=" Total Grade ", 
      ...
      caption=  paste("All succeful learners should be in the box\n","Date", FileDate) )+
...
> It is still difficult to measure precisely the time spent by learners on the LMS,  but **only `r NbLearners3hours`**  learners have spent more than 3 hours/Module on the course  *(`r round(100*NbLearners3hours/NbLearners,1)` \% )*.
...
mutate(TotalTime = TotalTime/60, 
 ...
 geom_hline (aes(yintercept = 3*(NbModules-1)*60), color = SIAP.color) +  # Adjust NbModules 
 annotate("text", x = 15, y = 3*(NbModules-1)*60 + 200,
          color= SIAP.color,
...
```{r}
# Compute the  time spent per activity

learners_Time  <- learners_Time %>%
  mutate(MinPerAct =  (as.numeric(TotalTime) / NbAct)/60 )

# We wil  remove some outliers
MinutesQ99 <-quantile(learners_Time$MinPerAct , 0.99)
MinutesQ50 <- round(median(learners_Time$MinPerAct),1)
```

 It is also to note that not all the participants spent the same amount of time on activities. A more detailed analysis would show which activities were the most time consuming as well as the distribution of time spend per module.  Still, we can observe that **half of the participants spent more than  `r MinutesQ50 `  minutes per activity**.[^Time2] 
 
[^Time2]: Computing the duration and time spent on each activity is not perfect, and exhibits a lot of variabilty depending on the type of activity.  

```{r}
# Distribution 
learners_Time %>%
  filter(MinPerAct < MinutesQ99) %>%
  ggplot() +
  aes(x = MinPerAct)+
  geom_histogram(bins = 100L, fill = SIAP.color, color = "white")+
  geom_vline(xintercept = MinutesQ50, linetype = "dashed", color = "red", size = 1) +
   annotate("text", x = 2*MinutesQ50 , y = NbCertificates/20,
          color= "red",
          label = paste0("median ", MinutesQ50, " minutes") )+
  ggtitle("Time per activity (Minutes)") +
  labs(x="Minutes per activity (estimated)")+
  theme_minimal()
...
The vertical axis on the graphic below reports the number of activities visited by learners, colored depending on success. Horizontally is the number of visits on the LMS(unique days). One expect to see very few reds in the upper and right sides of the graphic. 
...
Grades %>%
 ...
 annotate("text", y = 1, x = median(GradesPositive$NbDays) + 1,
          color= SIAP.color,
...
With **`r NbCertificates` certified learners**  out of  **`r paste0(NbLearners)`** registered participants from **`r NbCountries` different countries**, the course has certainly been useful for many, even if  **`r BestCountry`** with  **`r NbBestCountry`** certified participants represented the vast majority of our learners ( `r  round((100* as.numeric(NbBestCountry)/NbCertificates), 2)` %).  

```{r}
# counting the number of optional reports completed
NbProjects <- data.act %>%
  filter(if_all(contains("Assessment"), ~ . == "Completed")) %>%
  nrow()

  
# NbProjects <- data.act  %>%
#   filter(Your.turn...Work.with.a.data.set.and.use..Machine.Learning =="Completed") %>%
#   nrow()
```


The **mandatory** data-based exercise has been completed by  **`r NbProjects`** participants and peer-reviewed, 

The course was quite long ( `r as.numeric(CourseDuration)` days), but we have not experienced too much attrition, and a good engagement in forums. Many participants shared their resources (ML case-studies), providing us with some new materials to examine. Finally, the  success rate is  **`r RealSuccessRate`** %  and similar to previous editions.




```{r reportexport, include=FALSE}
