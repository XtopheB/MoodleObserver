...
StringIdentiyingGrades <- "Final"  # Can be "total"
...
Message1  <- paste("For better readability, countries with less than",MinNbLearners+1,"participants have been removed\n")

# Countries with too many participants 
Message2 <- paste("data from", temp_country$Country[1], "(",temp_country$Learners[1], "obs) has been removed\n")
...
GradesPositive %>%
 ...
 annotate("rect", xmin = GradeThreshold, xmax = GradeMax,
                  ymin = 0, ymax = Ymax,
...
course.log %>%
 ...
 filter(Date >= as.Date(CourseStartDate, format = "%d/%m/%Y")) %>%
  ...
  theme(panel.grid.major = element_blank(),
        ...
        axis.title.y = element_text(size = 8))+
  coord_fixed(ratio = 0.5 )  # can be  0.5
```
...
The optional data-based exercise has been completed by  **`r NbProjects`** participants and manually reviewed (*by your devoted lecturer* ;-) ), among which many files were not satisfactory or just random files uploaded in an attempt to gain some bonus points without producing anything. We also received very good and personalized reports, some using Python instead of the R template. 

The course was quite long (8 weeks), but we have not experienced too much attrition, and a good engagement in forums. Many participants shared their resources (ML case-studies), providing us with some new materials to examine. Finally, the  success rate is  **`r RealSuccessRate`** %  and similar to previous editions.
