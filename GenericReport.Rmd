...
ModStartIdx <- c(1,9,18, 28, 37, 47)  # Parameters for RAP!

##  Define the string that identify the Total Grade 
...
Message1  <- paste("For better readability, countries with less than",MinNbLearners,"participants have been removed\n")

# Countries with too many particpants 
...
Learners %>%
 filter(Country != "" | Country != "NA")%>%
  ...
  labs(x="Gender", y="Percentage of sucess", 
      caption = paste(NbStudentsNoShow, " participants with a grade of 0 (`noshows`) excluded \n
                     (date:", format(FileDate, "%A %d %B"),")"))+
  theme_minimal()+
  geom_hline( yintercept = RealSuccessRate/100,  color = "red" ) +
  theme(legend.position = "bottom")+ 
...
GradesPositive %>%
 ...
 annotate("text", x = GradeThreshold*1.2, y = Ymax*0.75,
        color= SIAP.color,
        label = "Success")+
 theme_minimal()
...
GradesPositive %>%
 filter(Country != "" | Country != "NA")%>%
  ...
  annotate("text", x = GradeThreshold+5, y = 0.75,
          color= "red",
          label = "Success")+
  theme_minimal()+
...
original activities proposed, showing a good commitment of active participants*.^[This number excludes the communication and administrative activities such as announcements, feedback, concept note, etc.]  
