...
# Dates of Modules opening MLOS22
Opening1 <- as_date(CourseStartDate)
Opening2 <- as_date("2022-11-28")
Opening3 <- as_date("2022-12-05")
Opening4 <- as_date("2022-12-12")
Opening5 <- as_date("2022-12-19")
Opening6 <- as_date("2022-12-19")
...
ggplot() +
 ...
 ggtitle("Learners registered by date sincer begining of the course (Cumulative)") +
...
names(module.act) <- c("Module name")
NbActivities <- nrow(module.act)

# Not used
# tabl.act <-  module.act[, .N, keyby = Actname2]
...
```{r}
module.act
kable(module.act, "latex", longtable = T, booktabs = T) %>%
...
> The "raw" success rate  is **`r round(100*NbCertificates/NbLearners, 1)`** %. However,  note that **`r nrow(StudentsNoShow)`**  participants have a grade of 0 (*no-shows*). So the "**real**"  success rate is  **`r round(100*NbCertificates/(NbLearners -nrow(StudentsNoShow)) , 1)`** %.
...
```
 ...
 > *The graphic exhibits a few access differences among the **`r length(unique(course.log$Actname))`** activities proposed*. 
...
Grades %>%
 ...
 labs(x="Nb of visits on the LMS (days)", 
...
df <- LearnersPaths  %>%
  filter(Certified == TRUE,
         Nbre_Steps >= NminStep ) %>%
  group_by(User.full.name) %>%
  select(User.full.name, ActNb, Actname, ModuleID_2, Certified, Country,
