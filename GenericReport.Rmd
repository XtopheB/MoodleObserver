...
actname <- actname %>%
            mutate(ModuleID = case_when(between(ActNb, ModStartIdx[1], ModStartIdx[2]-1) ~ 1,
                                          ...
                                          between(ActNb, ModStartIdx[8], ModStartIdx[9]-1) ~ 8,
...
names(module.tab) <- c("Module name","Module")
...
> The "raw" success rate  is **`r RawSuccessRate`** %. However,  note that **`r paste0(NbStudentsNoShow)`**  participants have a grade of 0 (*no-shows*). So the "**real**"  success rate is  **`r RealSuccessRate`** %.
...
```{r BarActivities, fig.height=10, fig.width=10, out.width = "80%"}
